...
export const selectionSort: AlgorithmDefinition = {
  ...
  for i from 0 to len(arr) - 2
    ...
    for j from i + 1 to len(arr) - 1
      #: comment "Comparing {$*arr[j]} with current minimum {$=arr[min]}: {arr[j] < arr[min] ? 'new minimum found' : 'current minimum unchanged'}"
      if arr[j] < arr[min]
        min = j
    #: comment "Minimum is {$=arr[min]} at index {$=min}. {min != i ? 'Swapping it into position ' : 'Already in position '}{$=i}"
    if min != i
      swap arr[i], arr[min]
    #: dim arr from 0 to i`,
